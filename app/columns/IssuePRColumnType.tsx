...
function issuePRSchema(multiple: boolean): OpenAI.ChatCompletionCreateParams['response_format'] {
  return {
    type: 'json_schema',
    json_schema: {
        ...
        properties: {
          [multiple ? 'references' : 'reference']: multiple
            ? {
                type: 'array',
                description:
                  'The issues or PRs that you have selected. If there are no clear references to select, use an empty array',
                items: {
                  type: 'object',
                  properties: {
                    number: { type: 'integer' },
            ...
            : {
                type: 'object',
                description:
                  'The issue or PR that you have selected. If there is no clear reference to select, use null',
                properties: {
                  number: { type: 'integer' },
                  repository: { type: 'string' },
                  type: { type: 'string', enum: ['issue', 'pull-request'] },
...
export const IssuePRColumnType: BaseColumnType<'issue-pr'> = {
    ...
    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
      {'references' in cell.response && cell.response.references ? (
        cell.response.references.length > 0 ? (
          cell.response.references.map((ref, index) => <IssueOrPR reference={ref} key={index} />)
        ) : (
  ...
  generateResponseSchema: (_, multiple?: boolean) => issuePRSchema(multiple || false),
